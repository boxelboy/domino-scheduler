define(function(require) {
    ...
    LoaderView = Backbone.View.extend({
        ...
        hide: function() {
            ...
            if(this.app.baseView == "timeline" && viewAlreadySet === false)
            {
                this.app.trigger('clickTimeLine');
            	viewAlreadySet = true;
            }
    ...
    return Backbone.View.extend({
        ...
        initialize: function(options) {
            ...
            this.loader = new LoaderView({ app : this.app});
            ...
            this.listenTo(this.app, 'menu:trigger:thisweek', this.thisWeekChange);
        ...
        onFilterChange: function(filter, resources) {
        	this.tmpConfigTimeline = scheduler.matrix.timeline;
            this.filter = filter;
        ...
        createResourceViews: function(timeline) {
            ...
            if (resources.length) {
				...
				/*
				Creates ODD Bug..not sure this is needed with current week button
				scheduler.date.timeline_start = _.bind(function(date)
				{
					if (scheduler.date.day_start(date).toString() == scheduler.date.day_start(new Date()).toString()) {
						date.setTime(this.startOfWeek);
        ...
        thisWeekChange : function()
        {
        	scheduler.setCurrentView(this.startOfWeek._d);
        },
        ...
        createEventText : function(schedule ){
        	var bookedFor,
        		jobID;



        	bookedFor =  (schedule.has('booked_for')) ? schedule.get('booked_for') : "";

            if(bookedFor == "")
            {
                bookedFor = (schedule.has('description')) ? schedule.get('description') : "";
            }

        	jobID 	  =  (schedule.has('job_number')) ? " [" + schedule.get('job_number') + "] " : "";

        	return bookedFor + jobID;
        },
        ...
        start: function() {
            ...
            scheduler._init_event = _.bind(function(event) {
                ...
                if(schedule.related('schedules:job').get('id')){
                	...
                	if(schedule.related('schedules:job').related('jobs:client'))
                	{
                		//Generate Tool Tip Text for Event Popup
	                	event.popupText = {};
	                	event.popupText.Company = schedule.related('schedules:job').related('jobs:client').has('company_name') ? schedule.related('schedules:job').related('jobs:client').get('company_name') : "";
	                	event.popupText.Phone = schedule.related('schedules:job').related('jobs:client').has('phone') ? schedule.related('schedules:job').related('jobs:client').get('phone') : "";
	                	event.popupText.Email = schedule.related('schedules:job').related('jobs:client').has('email') ? schedule.related('schedules:job').related('jobs:client').get('email') : "";
	                	event.popupText.Customer = schedule.related('schedules:job').has('customer_name') ? schedule.related('schedules:job').get('customer_name') : "";
                        event.popupText.Alias =schedule.related('schedules:job').related('jobs:client').has('alias') ? schedule.related('schedules:job').related('jobs:client').get('alias') : "";

                        if(event.popupText.Customer.charAt(0) == ","){
                            delete event.popupText.Customer;
                        }
                	}
                	event.text = this.createEventText(schedule);
          		}else{
          			event.text = (schedule.has('booked_for')) ? schedule.get('booked_for') : " No Event Text ";
          		}
          		...
          		if (typeof schedule.related('schedules:participants') !== 'undefined') {
					...
					//Generate Colours - We select first participant role..bit odd
					if(schedule.related('schedules:participants').models.length){
            ...
            scheduler.templates.tooltip_text = function(start,end,event) {
				if (typeof event.popupText == 'undefined') {
					...
					var html = "<b>Event:</b> " + event.text + "<br>";
					_.each(event.popupText, function(item, key){
						if(!_.isEmpty(item)){
							html += "<b>" + key + "</b> " + item + "<br>";
						}
					});

					html += "<b>Start date:</b> "+scheduler.templates.tooltip_date_format(start)+"<br/><b>End date:</b> "+scheduler.templates.tooltip_date_format(end);
                    return html;
		       	}
	       	};
            ...
            this.$el.dhx_scheduler({
                mode: (
                    typeof this.currentResource !== "undefined"  && this.app.baseView != "timeline" ?
                        (
                            (
                                this.currentResource.has('s_default_view') ? this.currentResource.get('s_default_view').toLowerCase() : this.app.baseView
                            )
                        ) : "month"
                    )
            });
                        ...
                        if(updatedEvent.resource_id != originalEvent.resource_id){
                            var participant = new ParticipantCollection();
                                participant.fetch({
                                    ...
                                    success : _.bind(function (collection)
                                    {
                                        var oldEventName = collection.models[0].get('event_name');
                                        var oldEventId = collection.models[0].get('event_id');
                                        var oldResourceCatId = collection.models[0].get('resource_category_id');
                                        ...
                                        participant.save().done(_.bind(function(){
                                            if(participant.get('resource_category_id') == oldResourceCatId)
                                            {
                                                participant.set('event_name', oldEventName);
                                                participant.set('role_id', oldEventId);
                                                participant.set('event_id', oldEventId);
                                                participant.save().done(_.bind(function(){
                                                }, this));
                                            }
                                        }, this));
			...
			//Sets height of bars, take a look @ .dhx_cal_event_clear, .dhx_cal_event_line { in css to.
            ...
            if (!this.startDate || !this.endDate || scheduler._min_date < this.startDate || scheduler._max_date > this.endDate || filterChanged || this.jobID || force) {
                ...
                if (this.jobID === false) {
                    data = {
                        ...
                        fields: '*,job.*,job.sales_order.*,job.sales_order.line_items.*,job.client.*,job.client.client_contacts.*,participants.*,participants.role.*,participants.resource.*,participants.resource.resource_category.*',
                        sort: 'start_date',
                        scheduled: 1,
